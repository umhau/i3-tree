...
def parsetreerecursive(parentcontainer,globaltreeitems_subset):

    print('parsing recursively')
    # note, problem: I don't think this travels back up the tree
    ...
    for childcontainer in parentcontainer.descendants():
        ...
        if childcontainer.parent == parentcontainer:
            ...
            if not childcontainer.window:
                ...
                # add a new subset to the tree
                globaltreeitems_subset.append([])

                # draw a placeholder for the container
                # globaltreeitems.append(TreeNode('↪'))
                globaltreeitems_subset[-1].addChild(TreeNode('↪'))

                # recursively parse
                parsetreerecursive(childcontainer,globaltreeitems_subset[-1])
                
            # it's a window, so draw it
            else:
                globaltreeitems_subset[-1].addChild(TreeNode(childcontainer.name))
    ...
    for topleveldesc in workspacetree.descendants():
        if topleveldesc.parent.parent.parent == workspacetree:
            ...
            if not topleveldesc.window:
                ...
                parsetreerecursive(topleveldesc,globaltreeitems[-1])
