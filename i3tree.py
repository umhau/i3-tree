...
row_color_inactive  = "#33322f"
row_color_active    = "#33322f"
gradient_color      = "#494844"
font_color          = "#a8a89c"  #"#dcdccc"
font_color_active   = "#36E592" #"#B3BEFF" # "#dcdccc" # "#B3BEFF" # "#494844"
font                = "APL385"

font_size           = "8pt"
row_vertical_size   = 24
container_vertical  = 12
gradient_center     = int(row_vertical_size/2)
container_grad_center = int(container_vertical/2)
...
async def main():
    ...
    # gradient.setColorAt(0.0, QColor(gradient_color))
    gradient.setColorAt(0.0, QColor(row_color_inactive))
    # gradient.setColorAt(1.0, QColor(row_color_inactive))
    gradient.setColorAt(1.0, QColor(gradient_color))
    brush = QBrush(gradient)

    gradient = QLinearGradient(0, 0, 0, container_grad_center)
    gradient.setSpread(QLinearGradient.ReflectSpread)
    # gradient.setColorAt(0.0, QColor(gradient_color))
    gradient.setColorAt(0.0, QColor(row_color_inactive))
    # gradient.setColorAt(1.0, QColor(row_color_inactive))
    gradient.setColorAt(1.0, QColor(gradient_color))
    thin_brush = QBrush(gradient)
    ...
    async def updatetree(conn, event):
        ...
        # # program frequently encounters errors with the i3 tree json stream.
        # # use this to try and diagnose.
        # os.system("rm -rf /tmp/i3tree*")
        # os.system("i3-msg -t get_tree > /tmp/i3tree.`date +%s`.json")
        # tree = await i3.get_tree()

        try:
            # saving the tree with the command line utility seems to improve the
            # json parsing rate
            os.system("i3-save-tree > /dev/null")
            tree = await i3.get_tree()
        except:
        ...
        # find the current active window (highlight it)
        ...
        for i3container in root_container.descendants():
            ...
            # if this is the active window, decorate differently

            # add visual gradient
            # rowitem.setBackground(brush)
            # rowitem.setBackground(QColor(0,0,0))
            

            if i3container == active_window:
                # print("active window: " + str(i3container.name))
                # rowitem.setBackground(QColor(row_color_active))
                rowitem.setForeground(QColor(font_color_active))


            if str(i3container.name) == 'None':
                rowitem.setBackground(thin_brush)
                # rowitem.setBackground(QColor(row_color_inactive))
                rowitem.setSizeHint(QtCore.QSize(20,container_vertical))
            else:
                rowitem.setBackground(brush)
                # rowitem.setBackground(QColor(row_color_inactive))
